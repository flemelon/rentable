...
import com.rentable.dms.config.KafkaProperties;
...
class DmsKafkaResourceIT {
    ...
    void setup() {
        ...
        DmsKafkaResource kafkaResource = new DmsKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc
            .perform(post("/api/dms-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/dms-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
